...
from bdsim.core.components import TransferBlock, block
...
class MultiRotor(TransferBlock):
    ...
    def __init__(self, model, *inputs, groundcheck=True, speedcheck=True, x0=None, **kwargs):
        ...
        for i in range(0, self.nrotors):
            ...
            self.D[:, i] = np.r_[model['d'] *
                                 cos(theta), model['d'] * sin(theta), model['h']]
    ...
    def deriv(self):
        ...
        for i in range(0, self.nrotors):  # for each rotor
            ...
            # Flapping
            beta = np.array([
                [((8/3*model['theta0'] + 2 * model['theta1']) * mu - 2 *
                  lc * mu) / (1 - mu**2 / 2)],  # Longitudinal flapping
...
if __name__ == "__main__":
    ...
    exec(open(os.path.join(pathlib.Path(
        __file__).parent.absolute(), "test_robots.py")).read())
