...
class FunctionBlockTest(unittest.TestCase):
    ...
    def test_gain(self):
        ...
        nt.assert_array_almost_equal(
            block._eval(np.r_[1, 2, 3])[0], np.r_[2, 4, 6])
        ...
        nt.assert_array_almost_equal(
            block._eval(2)[0], np.array([[2, 4], [6, 8]]))
        ...
        block = Gain(np.array([[1, 2], [3, 4]]))
        nt.assert_array_almost_equal(block._eval(np.array([[5, 6], [7, 8]]))[
                                     0], np.array([[23, 34], [31, 46]]))

        block = Gain(np.array([[1, 2], [3, 4]]), premul=True)
        nt.assert_array_almost_equal(block._eval(np.array([[5, 6], [7, 8]]))[
                                     0], np.array([[19, 22], [43, 50]]))
    ...
    def test_clip(self):
        ...
        nt.assert_array_equal(block._eval(
            np.r_[-10, -2, -1, 0, 1, 3, 10])[0], np.r_[-2, -2, -1, 0, 1, 3, 3])
    ...
    def test_function(self):
        ...
        block = Function(lambda x, y, a=0, b=0: x+y+a+b,
                         nin=2, kwargs={'a': 3, 'b': 4})
        self.assertEqual(block._eval(1, 2)[0], 10)
    ...
    def test_interpolate(self):
        ...
        block = Interpolate(x=np.r_[0, 5, 10], y=np.r_[
                            0, 1, 0].reshape((3, 1)))
        self.assertEqual(block._eval(0)[0], 0)
        self.assertEqual(block._eval(2.5)[0], 0.5)
        self.assertEqual(block._eval(5)[0], 1)
