...
from ...tuning.parameter import HyperParam, VecParam, Param
...
class TcpClientTuner(Tuner):
    ...
    def register_video_stream(self, feed_fn, name):
        ...
        def named_feed(): return feed_fn()
        named_feed.__name__ = name

        self.stream_app.route('/' + name)(named_feed)
        self.video_streams.append("http://%s:%d/%s" %
                                  (self.ip, self.stream_port, name))
    ...
    def register_signal_scope(self, name, n_signals, styles=None, labels=None):
        ...
        self.signal_scopes.append(
            {'name': name, 'n': n_signals, 'styles': styles, 'labels': labels})
        return id
    ...
    def setup_param_map(self, params: List[Param]):
        for param in params:
            ...
            def gui_reconstructor(param):
                msgpack.pack(self.get_param_defs(
                    [param], subparams=False), self.stream)
                self.stream.flush()
